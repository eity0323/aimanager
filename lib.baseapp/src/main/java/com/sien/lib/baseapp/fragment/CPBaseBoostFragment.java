 ...
 * @author sien
 * @date 2016/8/25
 * @descript 基类fragment(提供全局加载效果、无网络状态效果 -----不要与CPBaseBoostActivity同时使用)
 *
 * 注：基类管理presenter的生命周期，布局完全由子类管理，提供了统一调用独立开发模块页面功能（使用配置文件配置插件模块，插件模块对宿主透明，仅开发配置文件中提供的接口）
 * 注：子类不能覆盖onCreateView方法，需要通过getContentLayoutResId方法设置布局文件，此外子类不需要主动调用initViews，initial方法
 */
public abstract class CPBaseBoostFragment extends Fragment implements IPluginOrAloneChecker,IActivityOperater,ICPBaseBoostViewModel {
    ...
    private boolean needNetworkBar = true;//是否需要无网络提示状态栏
    ...
    private boolean toolbarViewStubed = false;//toolbar布局文件是否已经渲染
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        if (needOverlay) {
            rootView = inflater.inflate(R.layout.fragment_base_boost_overlay,container,false);
        }else {
            rootView = inflater.inflate(R.layout.fragment_base_boost,container,false);
        }
